...
router.get('/recommended', function (req, res) {
  ...
  var querystring =
                  "Select ad.ActivityID,ad.Title,Ad.City,Ad.Country,ap.CardImage FROM `activity-details` as ad RIGHT JOIN(	Select ActivityID FROM(	Select DISTINCT NewTemp.User1,NewTemp.UserSimilarity,ActivityID FROM `user-favourites` as US RIGHT JOIN("+
                  "Select * From (	Select Half1.User1,Half1.UserSimilarity FROM(	SELECT FinalIntersectionTable.IntersectionUser1 as User1,FinalIntersectionTable.IntersectionUser2 as User2,ROUND((FinalIntersectionTable.IntersectionCount)/(FinalUnionTable.UnionCount)*100,1) AS UserSimilarity "+
                  "FROM ( SELECT IntersectionTable.TempIntUser1 as IntersectionUser1, IntersectionTable.TempIntUser2 as IntersectionUser2,COUNT(*) as IntersectionCount FROM (SELECT TempIntersection.user1 AS TempIntUser1, TempIntersection.user2 AS TempIntUser2,Count(*) AS TempIntersectionCount "+
                  "FROM (SELECT UserListIntersection1.user1 AS user1, UserListIntersection1.user2  AS user2,favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT a.UserID AS user1,b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID "+
                  ") AS UserListIntersection1 ON favs.UserID = UserListIntersection1.user1 UNION ALL SELECT UserListIntersection2.user1 AS user1,UserListIntersection2.user2  AS user2, favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT a.UserID AS user1,b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a "+
                  "JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID) 	AS UserListIntersection2	ON favs.UserID = UserListIntersection2.user2	)TempIntersection GROUP BY TempIntersection.user1, TempIntersection.user2, TempIntersection.userLiked HAVING COUNT(*) >1 ORDER BY TempIntersection.user1, TempIntersection.user2, TempIntersection.userLiked	) AS IntersectionTable GROUP BY IntersectionTable.TempIntUser1, IntersectionTable.TempIntUser2, IntersectionTable.TempIntersectionCount) "+
                  "FinalIntersectionTable LEFT JOIN (	SELECT UnionTable.TempUnionUser1 AS UnionUser1,UnionTable.TempUnionUser2 AS UnionUser2,Count(DISTINCT userLiked) AS UnionCount FROM (SELECT UserListUnion1.user1 AS TempUnionUser1,UserListUnion1.user2  AS TempUnionUser2,favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs "+
                  ...
                  "FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT a.UserID AS user1,b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID ) 	AS UserListUnion2 ON favs.UserID = UserListUnion2.user2	) AS UnionTable	GROUP BY UnionTable.TempUnionUser1, UnionTable.TempUnionUser2	) AS FinalUnionTable "+
                  "	ON FinalIntersectionTable.IntersectionUser1=FinalUnionTable.UnionUser1 	AND FinalIntersectionTable.IntersectionUser2 = FinalUnionTable.UnionUser2 Where 	FinalIntersectionTable.IntersectionUser1="+ req.query.UserID +" 	or FinalIntersectionTable.IntersectionUser2="+ req.query.UserID +"	ORDER BY UserSimilarity DESC		)Half1 "+
                  "UNION Select	Half2.User2,	Half2.UserSimilarity	FROM(	SELECT	FinalIntersectionTable.IntersectionUser1 as User1,	FinalIntersectionTable.IntersectionUser2 as User2,   ROUND((FinalIntersectionTable.IntersectionCount)/(FinalUnionTable.UnionCount)*100,1) AS UserSimilarity "+
                  "FROM (	SELECT	IntersectionTable.TempIntUser1 as IntersectionUser1, IntersectionTable.TempIntUser2 as IntersectionUser2,	COUNT(*) as IntersectionCount	FROM (SELECT TempIntersection.user1 AS TempIntUser1, TempIntersection.user2 AS TempIntUser2,Count(*) AS TempIntersectionCount "+
                  "FROM (	SELECT UserListIntersection1.user1 AS user1,UserListIntersection1.user2  AS user2,favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT a.UserID AS user1,	b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a	JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID "+
                  ") 	AS UserListIntersection1	ON favs.UserID = UserListIntersection1.user1 UNION ALL SELECT	UserListIntersection2.user1 AS user1,UserListIntersection2.user2  AS user2, favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs "+
                  "RIGHT JOIN (SELECT a.UserID AS user1,	b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID) AS UserListIntersection2 ON favs.UserID = UserListIntersection2.user2 "+
                  ")TempIntersection	GROUP BY TempIntersection.user1, TempIntersection.user2, TempIntersection.userLiked	HAVING COUNT(*) >1 ORDER BY TempIntersection.user1, TempIntersection.user2, TempIntersection.userLiked "+
                  ") AS IntersectionTable GROUP BY IntersectionTable.TempIntUser1, IntersectionTable.TempIntUser2, IntersectionTable.TempIntersectionCount ) FinalIntersectionTable   "+
                  "LEFT JOIN (SELECT UnionTable.TempUnionUser1 AS UnionUser1,UnionTable.TempUnionUser2 AS UnionUser2,Count(DISTINCT userLiked) AS UnionCount FROM (SELECT UserListUnion1.user1 AS TempUnionUser1,UserListUnion1.user2  AS TempUnionUser2,favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT a.UserID AS user1,b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID "+
                  ") AS UserListUnion1 ON favs.UserID = UserListUnion1.user1 UNION ALL SELECT	UserListUnion2.user1 AS user1,UserListUnion2.user2  AS user2,favs.ActivityID AS userLiked FROM heroku_2e52a7e26390f81.`user-favourites` AS favs RIGHT JOIN (SELECT	a.UserID AS user1,b.UserID  AS user2 FROM heroku_2e52a7e26390f81.`users` as a JOIN heroku_2e52a7e26390f81.`users` as b ON a.UserID < b.UserID "+
                  ") AS UserListUnion2 ON favs.UserID = UserListUnion2.user2) AS UnionTable GROUP BY UnionTable.TempUnionUser1, UnionTable.TempUnionUser2) AS FinalUnionTable ON FinalIntersectionTable.IntersectionUser1=FinalUnionTable.UnionUser1 AND FinalIntersectionTable.IntersectionUser2 = FinalUnionTable.UnionUser2 Where FinalIntersectionTable.IntersectionUser1="+ req.query.UserID +" or FinalIntersectionTable.IntersectionUser2="+ req.query.UserID +
                  " ORDER BY UserSimilarity DESC)Half2	) Agg Where NOT Agg.User1="+ req.query.UserID +" ORDER BY UserSimilarity DESC		) as NewTemp 		on US.UserID=NewTemp.User1		) as Unordered		Group by ActivityID	order by Count(ActivityID) desc Limit 20 "+
                  ") ActivityList ON  ad.ActivityID=ActivityList.ActivityID   LEFT JOIN `activity-pictures` as ap ON ActivityList.ActivityID=ap.activityID;";
